...
(defun my/exwm--set-cwd ()
  (let* ((id (exwm--buffer->id (current-buffer)))
    ...
    (if (and cwd (file-accessible-directory-p cwd))
      ...
      (setq-local default-directory user-home-directory))))
#+end_src
*** Start apps
**** Rofi
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
(defvar my/exwm-char-mode-class-names '()
  "List of applications by char name that should use char mode instead of insert mode by default")

(defun my/exwm-enter-insert-or-char-mode ()
  (interactive)
  (my/exwm-enter-char-mode)
  ;; (if (member exwm-class-name my/exwm-char-mode-class-names)
  ;;     (my/exwm-enter-char-mode)
  ;;   (my/exwm-enter-insert-state))
  )
#+end_src
...
(defun my/exwm-set-global-keys ()
  ...
  (evil-define-key 'normal exwm-mode-map (kbd "i") 'my/exwm-enter-insert-or-char-mode)
  (evil-define-key 'normal exwm-mode-map (kbd "<down-mouse-1>" ) 'my/exwm-enter-insert-or-char-mode)
  (evil-define-key 'normal exwm-mode-map (kbd "<down-mouse-2>" ) 'my/exwm-enter-insert-or-char-mode)
  (evil-define-key 'normal exwm-mode-map (kbd "<down-mouse-3>" ) 'my/exwm-enter-insert-or-char-mode)
  )
#+end_src
*** Simulation keys
#+begin_src emacs-lisp
...
(use-package! exwm
  ...
  (add-hook 'exwm-manage-finish-hook #'my/exwm-enter-insert-or-char-mode)
...
;;   +format-with-lsp nil)
(add-hook! '(typescript-tsx-mode typescript-mode) #'tree-sitter-hl-mode :append :local (setq +format-with-lsp nil))
